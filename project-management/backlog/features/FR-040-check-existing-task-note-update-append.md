# Feature Request: FR-040 - Check if Task or Note Exists, Offer Update or Append
...
- [ ] Before creating a Joplin note, search for similar notes via FR-026 `NoteIndex.search()` (all notes)
...
- **User control**: Aligns with Stoic's replace/append pattern (BF-008); users appreciate being asked
...
**Notes**:
- Use **FR-026 Semantic Search** (`NoteIndex.search()`) for similarity detection—already implemented
...
- [FR-026: Semantic Search and Q&A](FR-026-semantic-search-qa.md) — use `NoteIndex.search()` for note similarity
- [BF-008: Stoic Evening Deletes Morning](../bugs/BF-008-stoic-evening-deletes-morning.md) — replace/append pattern
- [BF-005: Stoic Duplicate Session](../bugs/BF-005-stoic-journal-timezone-and-data-loss.md) — duplicate detection
- [FR-023: Intelligent Content Routing](FR-023-intelligent-content-routing.md) — note/task creation flow
...
- `src/note_index.py` — `NoteIndex.search()` (FR-026 semantic search)
