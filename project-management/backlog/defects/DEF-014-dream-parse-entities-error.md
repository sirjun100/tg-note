# Defect: DEF-014 - /dream Parse Entities Error (BadRequest)
...
Added `_dream_analysis_to_plain()` to strip Markdown. On `BadRequest` (parse/entities), fall back to plain text so the user always receives the analysis. Same pattern as DEF-010.

## References

- [DEF-010: Greeting Parse Entities Error](DEF-010-greeting-parse-entities-error.md) — similar fix: HTML mode + plain-text fallback
