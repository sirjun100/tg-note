# Defect: DEF-022 - /find Command Error in Fly.io Logs
...
2. **Telegram Markdown parse errors**: Search results are sent with `parse_mode="Markdown"`. Note titles, folder names, and snippets come from user content (Joplin) and can contain Markdown-special characters (`*`, `_`, `` ` ``, `[`, `]`). Unescaped characters cause:
   ...
   - Same pattern as DEF-010 (greeting), DEF-014/DEF-016 (dream).
...
2. **Safe message send**: Switch to `parse_mode="HTML"` with `html.escape()` for user content (titles, snippets, folder names). HTML mode avoids underscore/asterisk parse issues. Add plain-text fallback on send error (same pattern as DEF-010).
...
- [DEF-010: Greeting Parse Entities Error](DEF-010-greeting-parse-entities-error.md) — HTML mode + plain-text fallback
- [DEF-014: /dream Parse Entities Error](DEF-014-dream-parse-entities-error.md)
- [US-029: Quick Note Search](../user-stories/US-029-quick-note-search.md)
...
**Sprint 15**: See [sprint-15-implementation-guide.md](../../sprints/sprint-15-implementation-guide.md) § DEF-022 for verification steps and unit test scenarios. Implementation may already exist in `src/handlers/search.py`.
