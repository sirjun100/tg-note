# Defect: DEF-023 - /ask Command Crashes on Certain Prompts
...
- **Feature**: US-026 Semantic Search Q&A
...
- Same pattern as DEF-010 (greeting), DEF-014/DEF-016 (dream), DEF-022 (find).
...
- **Message too long**: Answer + sources could exceed Telegram's 4096-char limit (DEF-019).

## Solution

1. **Safe message send** (primary): Switch to `parse_mode="HTML"` with `html.escape()` for the answer and source titles, or add a try/except with plain-text fallback when sending (same pattern as DEF-010, DEF-022).
2. **Split long messages**: If response exceeds 4096 chars, split into multiple messages (DEF-019 pattern).
...
- [DEF-010: Greeting Parse Entities Error](DEF-010-greeting-parse-entities-error.md) — HTML + plain-text fallback
- [DEF-014: /dream Parse Entities Error](DEF-014-dream-parse-entities-error.md)
- [DEF-022: /find Command Error](DEF-022-find-command-flyio-error.md) — same fix pattern
- [US-026: Semantic Search Q&A](../user-stories/US-026-semantic-search-qa.md)
...
**Sprint 15**: See [sprint-15-implementation-guide.md](../../sprints/sprint-15-implementation-guide.md) § DEF-023 for exact code changes: `html.escape`, `_send_ask_response_safe`, `split_message_for_telegram`. Copy pattern from `src/handlers/search.py` `_send_search_results_safe`.
