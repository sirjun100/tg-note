...
**Sprint Goal**: Fix production bugs (/find, /ask) and add project creation command (/project_new), establishing a stable base and project workflow before project sync (US-034).

**Duration**: 2026-05-19 - 2026-06-01 (2 weeks, after Sprint 14)
**Status**: ✅ Completed
**Team Velocity**: 12 points (DEF-022 + DEF-023 + US-044 + US-039)
...
- Copy-paste patterns from existing fixes (DEF-010, DEF-022, DEF-019)
...
**Bug Fixes** (4 pts):
- DEF-022: /find command — Fly.io errors, Markdown parse; fix with get_folders try/except, HTML escape
- DEF-023: /ask command — crashes on certain prompts; fix with HTML escape, plain-text fallback

**Features** (8 pts):
- US-044: /project_new — create project with default PARA folders
- US-039: Star on task — treat ⭐/* as high priority in reports
...
**Key Deliverables**:
- DEF-022: /find returns search results without Fly.io errors
- DEF-023: /ask returns AI answers without crashing on Markdown-special characters
- US-044: `/project_new <name>` creates project with Overview, Backlog, Execution, Decisions, Assets, References
- US-039: Tasks with ⭐/* at start ranked as high priority in reports

**Dependencies**:
- US-005 (Joplin REST API) ✅
- US-026 (Semantic Search) ✅ for /ask
- US-029 (Quick Note Search) ✅ for /find
- ReorgOrchestrator (US-016) ✅ for US-044

**Risks & Blockers**:
- DEF-022, DEF-023: Same Markdown parse pattern as DEF-010, DEF-014 — HTML mode + escape + plain-text fallback
...
- [ ] All backlog items (DEF-022, DEF-023, US-044, US-039) refined and ready for development
...
**Reference**: [DEF-022](../backlog/defects/DEF-022-find-command-flyio-error.md)

**Technical References**:
- `src/handlers/search.py` — `_find()` (lines 84–164), `get_folders()` (118–124), `_send_search_results_safe` (53–76)
- Copy pattern from: `src/handlers/core.py` `_send_greeting_safe` (DEF-010)
...
|---------|------------------|------------------------|---------------------|--------|--------|----------|
| T-001 | Wrap get_folders() in try/except; use empty folder map on failure | `handlers/search.py:_find` | DEF-022 | ⭕ | 0.5 | — |
| T-002 | Switch to parse_mode="HTML" with html.escape() for titles, snippets, folder names | `handlers/search.py` | DEF-022 | ⭕ | 1 | — |
| T-003 | Add plain-text fallback on send error; verify in production | `handlers/search.py` | DEF-022 | ⭕ | 0.5 | — |
...
- [ ] Plain-text fallback on send error (same pattern as DEF-010, DEF-022)
- [ ] Long responses split if > 4096 chars (DEF-019 pattern)
- [ ] `/ask how did I learn how to cook` returns answer without crash

**Reference**: [DEF-023](../backlog/defects/DEF-023-ask-command-crash.md)
...
| Task ID | Task Description | Class/Method Reference | Document Reference | Status | Points | Assignee |
|---------|------------------|------------------------|---------------------|--------|--------|----------|
| T-004 | Switch to HTML mode with html.escape() for answer and source titles | `handlers/ask.py:_ask` | DEF-023 | ⭕ | 1 | — |
| T-005 | Add plain-text fallback on send error; split long messages (>4096 chars) | `handlers/ask.py` | DEF-023 | ⭕ | 0.5 | — |
| T-006 | Verify `/ask how did I learn how to cook` in production | — | DEF-023 | ⭕ | 0.5 | — |
...
**Reference**: [US-044](../backlog/user-stories/US-044-project-new-command.md)
...
| Task ID | Task Description | Class/Method Reference | Document Reference | Status | Points | Assignee |
|---------|------------------|------------------------|---------------------|--------|--------|----------|
| T-007 | Add create_project_with_default_folders() or extend ReorgOrchestrator | `reorg_orchestrator.py` | US-044 | ⭕ | 2 | — |
| T-008 | Register /project_new and /pn handlers; whitelist check | `handlers/reorg.py` or new handler | US-044 | ⭕ | 1 | — |
| T-009 | Create initial Overview note with template; duplicate handling | `reorg_orchestrator.py`, handler | US-044 | ⭕ | 1.5 | — |
| T-010 | Add to greeting/help; unit tests | `handlers/__init__.py`, tests | US-044 | ⭕ | 0.5 | — |
...
**Reference**: [US-039](../backlog/user-stories/US-039-star-on-task-as-high-priority.md)
...
| Task ID | Task Description | Class/Method Reference | Document Reference | Status | Points | Assignee |
|---------|------------------|------------------------|---------------------|--------|--------|----------|
| T-011 | Add PriorityLevel.URGENT; create star-detection helper | `report_generator.py` | US-039 | ⭕ | 0.5 | — |
| T-012 | Apply star priority in create_google_task_item(); star wins over due date | `report_generator.py` | US-039 | ⭕ | 1 | — |
| T-013 | Preserve star in /task and content routing; update routing prompt | `handlers/core.py`, `llm_orchestrator.py` | US-039 | ⭕ | 1 | — |
| T-014 | Apply to all task displays; add /help and /task docs | `report_generator.py`, handlers | US-039 | ⭕ | 0.5 | — |
...
- **Week 1**: Story 1 (DEF-022) — verify/finish T-001–T-003; Story 2 (DEF-023) — T-004–T-006; Story 3 start (T-007)
- **Week 2**: Story 3 (US-044) — T-007–T-010; Story 4 (US-039) — T-011–T-014
