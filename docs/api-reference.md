...
| (URL preview images) | Generated by Gemini from page metadata | Free tier | `GEMINI_API_KEY` | Optional |
...
| **URL preview images** | Gemini (image model) | Generated from title/description; no screenshot service |
...
|------------|-------|-------------|-------------|
| **Image generation** | `gemini-2.5-flash-image` | Recipe images (`src/recipe_image.py`), URL preview images (`src/url_image.py`) | — |
...
**URL preview images (URL-based notes):**
- Preview images for link-based notes are **generated by Gemini** from website-extracted metadata (title, description, content type, extracted text), not from a screenshot service.
- Implemented in `src/url_image.py`; uses the same `gemini-2.5-flash-image` model and `GEMINI_API_KEY`. Rate limits apply (see Gemini section).
